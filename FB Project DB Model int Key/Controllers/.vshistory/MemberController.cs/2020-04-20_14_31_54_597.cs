...
namespace FB_Project_DB_Model_int_Key.Controllers
{
    ...
    public class MemberController : Controller
    {
        ...
        public async Task<IActionResult> Home()
        {
            ...
            var posts = dbContext.Posts.Where(p => p.UserId == user.Id || user.Friends.Any(f => f.FriendId == p.UserId)).OrderByDescending(d => d.Date).Include(u => u.User).Include(c => c.Comments).Include(p => p.PostLikes).ToList();
