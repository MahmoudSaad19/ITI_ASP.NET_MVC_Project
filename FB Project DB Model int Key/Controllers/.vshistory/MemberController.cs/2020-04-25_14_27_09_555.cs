...
namespace FB_Project_DB_Model_int_Key.Controllers
{
    ...
    public class MemberController : Controller
    {
        ...
        public async Task<IActionResult> Profile(int? id)
        {
            ...
            user.BirthDate.GetDateTimeFormats();
            var date = user.BirthDate.Date.ToString().Split(" ")[0];
            var datef = user.BirthDate.Date.GetDateTimeFormats()[0];
        ...
        public IActionResult Search(string name)
        {
            if (name == null)
            {
                string RequestUrl = HttpContext.Request.Headers["Referer"].ToString().Split("=")[0];
